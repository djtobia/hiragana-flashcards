  ...
  <v-container fluid class="fill-height justify-center">
    <v-card width="500px">
      ...
      <v-card-text v-if="!viewCorrect">
        ...
        <v-row v-if="showCorrect" justify="center"><v-col cols="12" class="text-center">You got that one right!</v-col>

          <v-col cols="12" class="text-center">The answer was {{ hiragana.character }}</v-col></v-row>
        ...
        <PossibleAnswers v-if="store.images.length && !showCorrect && !showIncorrect" :hiragana="hiragana"
          :incorrectAnswer="store.getIncorrectAnswer" @correct="correct" @incorrect="incorrect" />
        <v-row class="mt-10">
          <v-col class="text-center">
            <v-btn @click="store.setIncorrectAsImages" v-if="store.images.length === 0 && store.incorrect.length > 0"
              color="orange-darken-4" class="mb-3">
      ...
      <v-card-text v-else>
        <v-row>
          <v-col v-for="hiragana of store.correct.sort((a, b) => a.character.localeCompare(b.character))"
            :key="hiragana.character" cols="4">
